...
from py.ui   import show_error, show_info, show_warning, setup_window
...
class CadBlockPlacerWindow(forms.WPFWindow):
    ...
    def __init__(self, saved_rules):
        ...
        if not imports:
            show_error("No CAD Links found in the document.", title="CAD to Revit", exitscript=True)
    ...
    def do_place_all(self):
        ...
        try:
            ...
            with SafeTransaction(doc, "CBP Place Families"):
                ...
                with forms.ProgressBar(title='Placing families…', cancellable=True, step=5) as pb:
                    current = 0
                    for rule in valid:
                        blocks = [b for b in all_blocks if b.Layer == rule.Layer]
                        if not blocks: continue
                        ...
                        for blk in blocks:
                            ...
                            current += 1
                            pb.update_progress(current, total_blocks)

            # Transaction 2: Group (separate to avoid rollback on group error)
            if placed_count > 0:
                with SafeTransaction(doc, "CBP Group Families"):
                    for rule in valid:
                        rule_ids_inner = List[ElementId]([
                            eid for eid in all_placed
                            # We can't easily re-associate IDs here, so just skip grouping
                            # Grouping logic removed for safety; elements are already selected below
                        ])
                        break  # placeholder — grouping per-rule requires tracking above

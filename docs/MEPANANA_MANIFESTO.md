...
### 🔹 Bước 3: Chuẩn hóa Icon `icon.png` theo ROOT BENCHMARK: SCHEDULE LINK (`SL`)
Tất cả icon công cụ **BẮT BUỘC** phải tuân thủ số đo pixel của `Schedule Link (SL)` làm chuẩn gốc:
* **Canvas:** `256 x 256` pixel, nền trong suốt 100% (32-bit ARGB PNG).
* **Nội dung:** 2 - 3 chữ cái viết tắt in hoa (Ví dụ: `SL`, `FC`, `SM`, `C2R`).
* **Kích thước Bounding Box (PushButton lớn):**
  * **Chiều cao chuẩn ($H$):** Đúng **`160px`** (Tọa độ $Y=50 \rightarrow Y=210$). Top Margin: $50\text{px}$, Bottom Margin: $46\text{px}$.
  * **Chiều rộng chuẩn ($W$):** Nằm trong khoảng **`160px - 235px`** (Tỉ lệ thon gọn / condensed factor `0.80` căn giữa).
* **Typography & Kerning:**
  * **Font:** `Segoe UI Bold` (`segoeuib.ttf`), Size Pushbutton: $220\text{pt}$, StackButton: $150\text{pt}$.
  * **Tỉ lệ khung hình (Aspect Ratio):** Thu hẹp bề ngang (condensed factor $0.80$) giúp icon thon gọn, thanh thoát và đồng nhất nét chữ trên toàn bộ Ribbon.
  * **Khoảng cách chữ (Tracking/Kerning):** Khoảng cách dương tự nhiên **`+8px`** (Pushbutton) / **`+5px`** (StackButton).
* **Kích thước Icon nút xếp chồng (Stacked Button 3 hàng):**
  * Chiều cao chuẩn ($H$): **`107px`** ($Y=74 \rightarrow Y=181$). Font Size $150\text{pt}$, Slim factor $0.80$.
* **Dải màu Gradient 7 tầng đa sắc (Phương ngang Left-to-Right từ $X=0$ đến $X=255$):**
...
* **Lệnh tạo icon tự động chuẩn:**
  ```bash
  py lib/py/make_icon.py "<TEXT>" "<OUTPUT_PATH>"
  # Ví dụ PushButton: py lib/py/make_icon.py "SL" "mepanana.tab/Data.panel/Schedule Link.pushbutton/icon.png"
  # Ví dụ StackButton: py lib/py/make_icon.py "PD" "mepanana.tab/Modeling.panel/Sprinkler.stack/Pendent Sprinkler.pushbutton/icon.png" --stack
  ```
